...
type Logger() as this  =
    ...
    let agent = MailboxProcessor.Start (fun agent ->
        ...
        let rec loop(count) = async {
            ...
            | Log (level,message,t) -> 
                let count = count + 1
                let str = sprintf "%s [%s]: %s" (t.ToString("mm:ss.ffff")) (levelToString level) message
                printfn "%s" str
                return! loop(count)
            | Flush ->
                return! loop(count)
    ...
    member this.LevelEnabled level = level >= this.Level

module Log =

    let defaultLogger = new Logger()

    let isEnabled = defaultLogger.LevelEnabled
